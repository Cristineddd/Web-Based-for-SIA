...
export default function Dashboard() {
  ...
  const handleCreateExam = async (formData: ExamFormData) => {
    try {
      ...
      if (!user?.instructorId) {
        toast.error(
          "⚠️ Instructor ID not found. Please log out and log back in, or contact support.",
        );
        return;
      }
  ...
  return (
    <div className="page-container">
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Link href="/exams">
              <Button
                variant="ghost"
                size="sm"
                className="border-2 border-transparent hover:border-[#B38B00] hover:bg-[#B38B00]/10 transition-colors duration-200 text-[#166534]"
              >
                View all
              </Button>
            </Link>
            ...
            {loading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-16 bg-[#B38B00]/10 rounded-lg animate-pulse border-2 border-[#166534]"
                  />
                ))}
              </div>
      ...
      >
        <AlertDialogContent className="border-2 border-[#B38B00]">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2 text-[#B38B00]">
              <Archive className="w-5 h-5" />
              Duplicate Batch Detected
            </AlertDialogTitle>
            <AlertDialogDescription className="text-[#166534]">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setDuplicateExamData(null)}
              className="border-2 border-[#166534]/20 text-[#166534] hover:bg-[#166534]/5"
            >
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              ...
              className="bg-[#B38B00] text-white hover:bg-[#B38B00]/90"
            >
              Proceed Anyway
            </AlertDialogAction>
          </AlertDialogFooter>
