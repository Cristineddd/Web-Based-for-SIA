...
export type SortColumn = 'studentId' | 'studentName' | 'score' | 'percentage' | 'grade' | 'date';
...
export default function ExamScoresTable({
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [gradeFilter, statusFilter, dateFrom, dateTo, minPercentage, maxPercentage]);
  ...
  const stats = useMemo(() => {
    ...
    const passCount = data.filter((r) => r.percentage >= passingThreshold).length;
  ...
  const searchableStudents: SearchableStudent[] = useMemo(
    () => data.map((r) => ({ studentId: r.studentId, studentName: r.studentName, email: r.email })),
    [data],
  );
  ...
  const processedRows = useMemo(() => {
    ...
    return sorted;
  }, [data, debouncedSearchQuery, fuse, sortColumn, sortDirection, gradeFilter, statusFilter, dateFrom, dateTo, minPercentage, maxPercentage, passingThreshold]);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <Card className="p-4 border border-green-200 bg-green-50/50">
              ...
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-bold text-green-700">{stats.passCount}</p>
            ...
            <Card className="p-4 border">
              ...
              <p className="text-2xl font-bold text-[#1a472a]">{stats.highestPercentage}%</p>
            </Card>
            ...
            <Card className="p-4 border">
              ...
              <p className="text-2xl font-bold text-[#1a472a]">{stats.total >= 2 ? `${stats.lowestPercentage}%` : '—'}</p>
            </Card>
            ...
            <Card className="p-4 border">
              ...
              <p className="text-2xl font-bold text-[#1a472a]">{stats.total >= 2 ? `${stats.medianPercentage}%` : '—'}</p>
            </Card>
          </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex h-4 rounded-full overflow-hidden bg-gray-100">
                  {stats.passRate > 0 && (
                    ...
                    >
                      {stats.passRate >= 15 && (
                        <span className="text-[10px] font-bold text-white">{stats.passRate}%</span>
                      )}
                    </div>
                  )}
                  {stats.failRate > 0 && (
                    ...
                    >
                      {stats.failRate >= 15 && (
                        <span className="text-[10px] font-bold text-white">{stats.failRate}%</span>
                      )}
                    </div>
                  )}
                </div>
            ...
            {showThresholdConfig && (
              <div className="mt-4 pt-3 border-t">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <input
                      ...
                      value={localThreshold}
                      onChange={(e) =>
                        setLocalThreshold(Math.max(0, Math.min(100, Number(e.target.value) || 0)))
                      }
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Students scoring ≥ {localThreshold}% will be marked as passing.
                  {onThresholdChange
    ...
    <Card className="border overflow-hidden">
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <CardTitle className="text-lg font-semibold text-[#1a472a]">
            Scores — {processedRows.length} student{processedRows.length !== 1 ? 's' : ''}
          </CardTitle>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              className={showFilters ? 'bg-[#1a472a] hover:bg-[#2d6b47] text-white' : ''}
            >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="100"
                  value={maxPercentage}
                  onChange={(e) => { setMaxPercentage(e.target.value); setCurrentPage(1); }}
                  className="h-9 text-sm"
                />
              </div>
            </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {gradeFilter !== 'all' && (
                  <Badge variant="secondary" className="gap-1 text-xs">
                    Grade: {gradeFilter}
                    <button onClick={() => setGradeFilter('all')} className="ml-0.5 hover:text-red-600">×</button>
                  </Badge>
                )}
                {statusFilter !== 'all' && (
                  <Badge variant="secondary" className="gap-1 text-xs">
                    Status: {statusFilter === 'pass' ? 'Pass' : 'Fail'}
                    <button onClick={() => setStatusFilter('all')} className="ml-0.5 hover:text-red-600">×</button>
                  </Badge>
                )}
                {dateFrom && (
                  <Badge variant="secondary" className="gap-1 text-xs">
                    From: {dateFrom}
                    <button onClick={() => setDateFrom('')} className="ml-0.5 hover:text-red-600">×</button>
                  </Badge>
                )}
                {dateTo && (
                  <Badge variant="secondary" className="gap-1 text-xs">
                    To: {dateTo}
                    <button onClick={() => setDateTo('')} className="ml-0.5 hover:text-red-600">×</button>
                  </Badge>
                )}
                {minPercentage && (
                  <Badge variant="secondary" className="gap-1 text-xs">
                    Min: {minPercentage}%
                    <button onClick={() => setMinPercentage('')} className="ml-0.5 hover:text-red-600">×</button>
                  </Badge>
                )}
                {maxPercentage && (
                  <Badge variant="secondary" className="gap-1 text-xs">
                    Max: {maxPercentage}%
                    <button onClick={() => setMaxPercentage('')} className="ml-0.5 hover:text-red-600">×</button>
                  </Badge>
                )}
              </div>
            )}
      ...
      <CardContent className="p-0">
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow className="bg-[#fffde7] hover:bg-[#fffde7]">
                <TableHead className="w-[50px] text-[#1a472a] font-semibold">#</TableHead>
                <TableHead>
                  <button
                    className="inline-flex items-center text-[#1a472a] font-semibold hover:text-[#2d6b47]"
                ...
                <TableHead className="text-[#1a472a] font-semibold">Status</TableHead>
                {onViewStudent && (
                  <TableHead className="text-[#1a472a] font-semibold">Actions</TableHead>
                )}
              </TableRow>
            </TableHeader>
            ...
            <TableBody>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      {searchQuery ? (
                        ...
                        <>
                          <p className="font-medium">No results for {examTitle}</p>
              ...
              {!loading &&
                paginatedRows.map((row, idx) => {
                  ...
                  return (
                    <TableRow key={row.studentId} className="hover:bg-muted/50 transition-colors">
                      ...
                      <TableCell className="font-medium text-[#1a472a]">{row.studentName}</TableCell>
                          ...
                          <div className="hidden sm:block w-16 h-2 rounded-full bg-gray-200 overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all ${
                                row.percentage >= passingThreshold ? 'bg-green-500' : 'bg-red-400'
                              }`}
                      ...
                      <TableCell>
                        <Badge variant="outline" className={getGradeColorClass(row.grade)}>
                          {row.grade}
                        </Badge>
                      </TableCell>
              ...
              {pageNumbers.map((p, i) =>
                p === 'ellipsis' ? (
                  ...
                  <Button
                    ...
                    size="icon"
                    className={`h-8 w-8 ${
                      p === safePage ? 'bg-[#1a472a] text-white hover:bg-[#2d6b47]' : ''
                    }`}
                    onClick={() => goToPage(p)}
                  >
                    {p}
