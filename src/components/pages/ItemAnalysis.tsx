...
export default function ItemAnalysisPage({ params }: ItemAnalysisProps) {
      ...
      try {
        ...
        for (let i = 1; i <= examData.num_items; i++) {
          ...
          for (let j = 0; j < examData.choices_per_item; j++) {
            choiceDistribution[String.fromCharCode(65 + j)] =
          ...
          mockQuestions.push({
            ...
            choiceDistribution,
            totalResponses: Object.values(choiceDistribution).reduce(
              (a, b) => a + b,
              0,
            ),
          });
  ...
  if (!exam) {
    return (
      <div className="space-y-6">
        <Link
          href="/exams"
          className="p-2 hover:bg-muted rounded-md transition-colors inline-block"
        >
  ...
  const avgDiscrimination =
    questions.length > 0
      ? parseFloat(
          (
            questions.reduce((sum, q) => sum + q.discrimination, 0) /
            questions.length
          ).toFixed(2),
        )
      : 0;
    ...
    <div className="space-y-6">
              ...
              >
                ...
                <div className="mb-3">
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                    {Object.entries(q.choiceDistribution).map(
                      ([choice, count]) => {
                        const percentage =
                          q.totalResponses > 0
                            ? (count / q.totalResponses) * 100
                            : 0;
                        return (
                          <div key={choice} className="text-center">
                            <div className="w-full bg-muted rounded-md overflow-hidden mb-1">
                              <div
                                ...
                                style={{
                                  height: `${Math.max(40, percentage * 2)}px`,
                                }}
                              />
                            </div>
      ...
      <Card className="p-4 sm:p-6 border bg-blue-50 border-blue-200">
        ...
        <ul className="text-sm text-foreground space-y-1 ml-7">
          <li>
            • <strong>Correct Rate:</strong> Percentage of students who answered
            correctly
          </li>
          <li>
            • <strong>Difficulty:</strong> Easy (76-100%), Medium (40-75%), Hard
            (0-39%)
          </li>
          <li>
            • <strong>Discrimination:</strong> How well the question
            differentiates between high and low performers (0-1 scale)
          </li>
          <li>
            • <strong>Distribution:</strong> Visual representation of how
            students chose each answer
          </li>
        </ul>
      </Card>
    </div>
