...
export default function OMRScanner({ examId }: OMRScannerProps) {
  ...
  const [mode, setMode] = useState<
    "select" | "camera" | "upload" | "processing" | "review" | "results"
  >("select");
  ...
  const [multipleAnswerQuestions, setMultipleAnswerQuestions] = useState<
    number[]
  >([]);
    ...
    const runDetection = (timestamp: number) => {
      ...
      try {
        ...
        if (overlay) {
          ...
          if (oCtx) {
            overlay.width =
              overlay.offsetWidth * (window.devicePixelRatio || 1);
            overlay.height =
              overlay.offsetHeight * (window.devicePixelRatio || 1);
            oCtx.scale(
            ...
            if (markers.found) {
              ...
              oCtx.beginPath();
              oCtx.moveTo(
                markers.topLeft.x * scaleX,
                markers.topLeft.y * scaleY,
              );
              oCtx.lineTo(
                markers.topRight.x * scaleX,
                markers.topRight.y * scaleY,
              );
              oCtx.lineTo(
                markers.bottomRight.x * scaleX,
                markers.bottomRight.y * scaleY,
              );
              oCtx.lineTo(
                markers.bottomLeft.x * scaleX,
                markers.bottomLeft.y * scaleY,
              );
              oCtx.closePath();
              oCtx.stroke();
              oCtx.setLineDash([]);
  ...
  const preprocessCameraImage = (
    ...
    const paperThreshRow = medianRow * 0.7;
    const paperThreshCol = medianCol * 0.7;
    ...
    for (let y = 0; y < height; y++) {
      if (rowBrightness[y] > paperThreshRow) {
        top = y;
        break;
      }
    }
    ...
    for (let x = 0; x < width; x++) {
      if (colBrightness[x] > paperThreshCol) {
        left = x;
        break;
      }
    }
    ...
    if (cropW < width * 0.92 || cropH < height * 0.92) {
      ...
      if (croppedCtx) {
        ...
        console.log(
          `[Camera] Cropped image from ${width}x${height} to ${cropW}x${cropH} (paper region)`,
        );
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      const { studentId, answers, multipleAnswers, idDoubleShades } =
        await detectBubbles(
          imageData,
          exam.num_items,
      ...
      if (idDoubleShades.length > 0) {
        ...
        idError =
          "No Student ID was detected. Please check if the student properly shaded their ID bubbles.";
      } else if (!classData) {
        idError =
          "No class is linked to this exam. Please go to exam settings and assign a class before scanning.";
      } else {
      ...
      for (let i = 0; i < totalQuestions; i++) {
        if (
          answers[i] &&
          answerKey[i] &&
          answers[i].toUpperCase() === answerKey[i].toUpperCase()
        ) {
          score++;
        }
      }
  ...
  const findCornerMarkers = (
    ...
    const markerSizes = isCamera
      ? [
          ...
          Math.floor(baseMarkerSize * 1.4),
        ]
      : [baseMarkerSize];
    const searchFraction = isCamera ? 0.35 : 0.3;
    const minDensityThreshold = isCamera ? 0.3 : 0.4;
    ...
    console.log(
      `[OMR] Marker detection (${isCamera ? "camera" : "upload"}): densities TL=${tl.density.toFixed(2)}, TR=${tr.density.toFixed(2)}, BL=${bl.density.toFixed(2)}, BR=${br.density.toFixed(2)}, threshold=${minDensityThreshold}, found=${markersValid}`,
    );
  ...
  const mapToPixel = (
    ...
    const botX =
      ...
      nx * (markers.bottomRight.x - markers.bottomLeft.x);
    const botY =
      markers.bottomLeft.y +
      nx * (markers.bottomRight.y - markers.bottomLeft.y);
    return {
  ...
  const getTemplateLayout = (numQuestions: number): TemplateLayout => {
    const templateType =
      numQuestions <= 20 ? 20 : numQuestions <= 50 ? 50 : 100;
        ...
        answerBlocks: [
          {
            ...
            firstBubbleNX: 13 / fw,
            firstBubbleNY: 58 / fh,
            bubbleSpacingNX: 4.8 / fw,
            rowSpacingNY: 4.5 / fh,
          },
          {
            startQ: 11,
            endQ: 20,
            firstBubbleNX: 55.5 / fw,
            firstBubbleNY: 58 / fh,
            bubbleSpacingNX: 4.8 / fw,
            rowSpacingNY: 4.5 / fh,
          },
  ...
  const detectBubbles = async (
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        integral[y * width + x] =
          rowSum + (y > 0 ? integral[(y - 1) * width + x] : 0);
      }
    }
    ...
    if (isCamera) {
      ...
      console.log(
        `[OMR] Camera: ${width}x${height}, mean=${meanBrightness.toFixed(1)}, otsu=${globalThreshold}, offset=${adaptiveOffset}, block=${halfBlock}`,
      );
      ...
      console.log(
        `[OMR] Upload: ${width}x${height}, otsu=${globalThreshold}, block=${halfBlock}`,
      );
    ...
    console.log(
      "[OMR] Corner markers found:",
      markers.found,
      "TL:",
      Math.round(markers.topLeft.x),
      Math.round(markers.topLeft.y),
      "BR:",
      Math.round(markers.bottomRight.x),
      Math.round(markers.bottomRight.y),
    );
  ...
  const sampleBubbleGrayscale = (
    ...
    let count = 0;
    const rx = radiusX * 0.7;
    const ry = radiusY * 0.7;
    const step = Math.max(1, Math.floor(Math.min(rx, ry) / 6));
    ...
    for (
      let dy = -Math.floor(outerRY);
      dy <= Math.floor(outerRY);
      dy += outerStep
    ) {
      for (
        let dx = -Math.floor(outerRX);
        dx <= Math.floor(outerRX);
        dx += outerStep
      ) {
  ...
  const detectStudentIdFromImage = (
    ...
    console.log(
      `[ID] BubbleR: ${idBubbleRX.toFixed(1)}x${idBubbleRY.toFixed(1)}, mode=${isCamera ? "camera" : "upload"}`,
    );

    // Different thresholds for camera vs upload
    const ID_FILL_THRESHOLD = isCamera ? 0.1 : 0.25;
    ...
    for (let col = 0; col < 9; col++) {
      ...
      if (maxFill > ID_FILL_THRESHOLD) {
        ...
        if (filledCount > 1) {
          ...
          console.log(
            `[ID] ⚠️ Col ${col} has DOUBLE SHADE (${filledCount} bubbles filled)`,
          );
        }
      }

      console.log(
        `[ID] Col ${col}: fills=[${fills.map((f) => f.toFixed(3)).join(",")}] → ${hasDetection ? detectedDigit : "?"} (max=${maxFill.toFixed(3)})`,
      );
    ...
    console.log(
      "[ID] Raw digits:",
      raw,
      doubleShadeColumns.length > 0
        ? `(double-shade in columns: ${doubleShadeColumns.join(",")})`
        : "",
    );
  ...
  const detectAnswersFromImage = (
    ...
    // Different thresholds for camera vs upload
    const FILL_THRESHOLD = isCamera ? 0.08 : 0.2;
      ...
      for (let q = block.startQ; q <= block.endQ && q <= numQuestions; q++) {
        ...
        if (isCamera && maxFill > FILL_THRESHOLD && fills.length > 1) {
          const otherFills = fills
            .filter((f) => f.choice !== selectedChoice)
            .map((f) => f.fill);
          const avgOther =
            otherFills.reduce((a, b) => a + b, 0) / otherFills.length;
          if (maxFill < avgOther * 2.0 && maxFill < 0.15) {
            console.log(
              `[ANS] Q${q}: rejected weak detection ${selectedChoice}=${maxFill.toFixed(3)} (avgOther=${avgOther.toFixed(3)})`,
            );
            selectedChoice = "";
            maxFill = 0;
          }
        ...
        if (maxFill > FILL_THRESHOLD) {
          const filledBubbles = fills.filter(
            (f) =>
              f.fill > FILL_THRESHOLD && f.fill >= maxFill * MULTI_ANSWER_RATIO,
          );
          if (filledBubbles.length > 1) {
            multipleAnswers.push(q);
            console.log(
              `[MULTI] Q${q}: ${filledBubbles.map((f) => `${f.choice}=${f.fill.toFixed(3)}`).join(", ")} | all: ${fills.map((f) => `${f.choice}=${f.fill.toFixed(3)}`).join(", ")}`,
            );
          }
        }
  ...
  const saveScanResult = async () => {
    ...
    if (studentIdError) {
      toast.error(
        "Cannot save: Student ID is not registered in this class. Please correct the Student ID first.",
      );
      return;
    }
    if (idDoubleShadeColumns.length > 0) {
      toast.error(
        "Cannot save: Student ID has columns with multiple bubbles shaded. Please correct the Student ID first.",
      );
      return;
    }
    ...
    if (!classData) {
      toast.error(
        "Cannot save: No class is linked to this exam. Please assign a class to the exam first.",
      );
      setStudentIdError(
        "No class is linked to this exam. Please go to exam settings and assign a class before scanning.",
      );
    ...
    const student = classData.students.find(
      (s) => s.student_id === detectedStudentId,
    );
    if (!student) {
      toast.error(
        `Cannot save: Student ID "${detectedStudentId}" is not registered in class "${classData.class_name} - ${classData.section_block}".`,
      );
      setStudentIdError(
        `Student ID "${detectedStudentId}" is not registered in class "${classData.class_name} - ${classData.section_block}". Please verify the student is enrolled in this class.`,
      );
      return;
    }
  ...
  const editAnswer = (index: number, newValue: string) => {
    ...
    if (upper.length <= 1 && /^[A-Z]?$/.test(upper)) {
      const choiceLimit = String.fromCharCode(
        64 + (exam?.choices_per_item || 4),
      );
      if (!upper || upper <= choiceLimit) {
        ...
        for (let i = 0; i < totalQuestions; i++) {
          if (
            newAnswers[i] &&
            answerKey[i] &&
            newAnswers[i].toUpperCase() === answerKey[i].toUpperCase()
          ) {
            score++;
          }
        }
  ...
  if (!exam) {
    return (
      <div className="space-y-6">
        <Link
          href="/exams"
          className="inline-flex items-center gap-2 text-gray-600 hover:text-[#1a472a]"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#1a472a]">
              Scan Answer Sheets
            </h1>
            <p className="text-gray-600">
              {exam.title} • {exam.num_items} questions
            </p>
          </div>
        </div>
      ...
      {mode === "camera" && (
        <Card className="overflow-hidden">
          <div className="relative bg-black" style={{ aspectRatio: "3/4" }}>
            ...
            >
              <div
                className={`px-4 py-2 rounded-full text-xs font-semibold flex items-center gap-2 transition-all duration-300 ${
                  markersDetected
              ...
              >
                <div
                  className={`w-2.5 h-2.5 rounded-full ${markersDetected ? "bg-green-300 animate-pulse" : "bg-red-300"}`}
                />
                {markersDetected
                  ? "Ready to capture"
                  : "Align sheet — find all 4 corners"}
              </div>
            </div>
            {/* Bottom instruction */}
            <div
          ...
          <div className="p-4 flex justify-center gap-4">
            ...
            <Button
              onClick={() => {
                if (!markersDetected) {
                  toast.warning(
                    "Alignment marks not fully detected. Grading might be less accurate.",
                  );
                }
                capturePhoto();
              }}
            ...
            >
              ...
              {markersDetected ? "Capture" : "Capture Anyway"}
            </Button>
          </div>
        </Card>
      )}
          ...
          <div className="space-y-6">
            ...
            <div className="max-w-xs mx-auto">
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#1a472a] rounded-full animate-pulse"
                  style={{ width: "60%" }}
                />
              </div>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-orange-800">
                    Multiple Bubbles Shaded in Student ID
                  </h4>
                  <p className="text-sm text-orange-700 mt-1">
                    Column(s) <strong>{idDoubleShadeColumns.join(", ")}</strong>{" "}
                    of the Student ID have more than one bubble shaded. Each
                    column must have only <strong>one digit</strong> selected.
                  </p>
                  <p className="text-xs text-orange-600 mt-2">
                    Please ask the student to properly shade only one bubble per
                    column, or manually correct the Student ID below.
                  </p>
                </div>
              </div>
            </Card>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-600 mt-2">
                    You must correct the Student ID before saving. Edit the ID
                    field below or discard and re-scan.
                  </p>
                </div>
              </div>
            </Card>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700 mt-1">
                    The following question(s) have more than one bubble shaded:{" "}
                    <strong>
                      {multipleAnswerQuestions.map((q) => `#${q}`).join(", ")}
                    </strong>
                  </p>
                  <p className="text-xs text-yellow-600 mt-2">
                    Only one answer per question is allowed. The system selected
                    the darkest bubble, but please verify and correct if needed.
                    Remind the student to shade only one bubble per question.
                  </p>
                </div>
              </div>
            </Card>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!newId || /^0+$/.test(newId)) {
                          setStudentIdError(
                            "No Student ID provided. Please enter a valid Student ID.",
                          );
                          setMatchedStudent(null);
                        } else if (!classData) {
                          setStudentIdError(
                          ...
                          if (student) {
                            ...
                            setMatchedStudent(null);
                            setStudentIdError(
                              `Student ID "${newId}" is not registered in class "${classData.class_name} - ${classData.section_block}". Please verify the student is enrolled in this class or check if the ID was shaded correctly.`,
                            );
                          }
                        }
                      }}
              ...
              <div className="text-right">
                <div
                  className={`inline-block px-4 py-2 rounded-lg text-2xl font-bold ${getGradeColor(scanResult.letterGrade)}`}
                >
                ...
                <div className="space-y-6">
                  <div>
                    <p className="text-sm font-medium text-gray-500 mb-2">
                      Questions 1-{halfPoint}
                    </p>
                    <div className="grid grid-cols-5 sm:grid-cols-10 gap-3">
                      {firstRow.map((answer, i) => {
                        const isCorrect =
                          answerKey[i] &&
                          answer.toUpperCase() === answerKey[i].toUpperCase();
                        const hasMultiple = multipleAnswerQuestions.includes(
                          i + 1,
                        );
                        return (
                          <div key={i} className="text-center">
                            <span
                              className={`text-xs block mb-1 ${hasMultiple ? "text-yellow-600 font-bold" : "text-gray-500"}`}
                            >
                              {i + 1}
                            </span>
                  ...
                  {secondRow.length > 0 && (
                    <div className="pt-4 border-t">
                      <p className="text-sm font-medium text-gray-500 mb-2">
                        Questions {halfPoint + 1}-{detectedAnswers.length}
                      </p>
                      <div className="grid grid-cols-5 sm:grid-cols-10 gap-3">
                        {secondRow.map((answer, i) => {
                          ...
                          const isCorrect =
                            ...
                            answer.toUpperCase() ===
                              answerKey[actualIndex].toUpperCase();
                          const hasMultiple = multipleAnswerQuestions.includes(
                            actualIndex + 1,
                          );
                          return (
                            <div key={actualIndex} className="text-center">
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  value={answer}
                                  onChange={(e) =>
                                    editAnswer(actualIndex, e.target.value)
                                  }
                                  maxLength={1}
                                  className={`w-10 h-10 text-center font-bold rounded-lg border-2 transition-colors ${
                                    hasMultiple
            ...
            <div className="flex items-center gap-4 mt-6 pt-4 border-t text-sm flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">
                  Incorrect (correct answer shown below)
                </span>
              </div>
              {multipleAnswerQuestions.length > 0 && (
                <div className="flex items-center gap-2">
          ...
          <div className="flex justify-center gap-4">
            ...
            <Button
              onClick={() => {
                if (idDoubleShadeColumns.length > 0) {
                  toast.error(
                    "Student ID has multiple bubbles shaded. Please correct the ID before saving.",
                  );
                  return;
                }
                if (studentIdError) {
                  toast.error(
                    "Please correct the Student ID before saving. The student must be registered in this class.",
                  );
                  return;
                }
                saveScanResult();
              ...
              disabled={
                saving || !!studentIdError || idDoubleShadeColumns.length > 0
              }
            ...
            {recentScans.map((scan, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span
                    className={`px-2 py-1 rounded text-sm font-bold ${getGradeColor(scan.letterGrade)}`}
                  >
                    {scan.letterGrade}
                  </span>
                </div>
